...
pdftitle={TDT and snpMatrix Vignette}]
{hyperref}

\title{TDT vignette\\Use of snpMatrix in family--based studies}
...
The {\tt snpMatrix} package contains some tools for analysis of
...
of the {\tt snp.matrix} containing the genotype data. The following
commands read some illustrative data on 3,017 subject and 43
...
subject and pedigree information ({\tt pedfile}) and a {\tt
  snp.matrix} containing the genotype data ({\tt genotypes}):
<<family-data>>=
require(snpMatrix)
data(families)
head(genotypes)
...
the convention in the {\tt LINKAGE} package, which would code these as
...
subject and plot its frequency distribution:
...
contribution of teh two trios in each family to the test statistic
