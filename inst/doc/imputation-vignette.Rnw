...
``tag'' SNPs
have been chosen, the haplotypes of these SNPs are phased and a
logistic regression prediction equation calculated. 
Where prediction does not achieve a target $R^2$, 
phased haplotype frequencies are estimated under a ``saturated'' model
({\it i.e. } a model with one parameter for the frequency of each
identified haplotype). When the gain in $R^2$ exceeds a threshold, 
this rule is saved in preference to the logistic regression
based rule. The $R^2$ target and threshold which control this process are 
supplied in the argument {\tt use.haps}.\footnote{Ideally we would
  search intermediate models between the logistic regression ``main
  effects'' model and the fully saturated model. This is planned for a
...
to store imputed SNPs in an object of class {\tt snp.matrix} (or {\tt
  X.snp.matrix}). The posterior probabilities of assignment of each
...
how {\tt snpMatrix} stores uncertain genotypes is provided by the
...
Note that the ability of {\tt snpMatrix} to store imputed genotypes in
...
(which is stored in the {\tt data} sub-directory of 
the {\tt snpMatrix} package)
contains imputed SNPs generated by the MACH program, using the {\tt
  --mle} and {\tt --mldetails} options. In the following commands, we
read the file and inspect one the imputed SNP in column~50:  
<<mach,fig=TRUE>>=
mach <- read.mach(system.file("data/mach1.out.mlprob.gz", package="snpMatrix"))
