...
pdftitle={Imputed SNP analyses with chopsticks}]
{hyperref}

\title{Imputed SNP analyses and meta-analysis with chopsticks}
...
use of {\tt chopsticks} for imputation,
...
training data set, select alternate SNPs to be potentially
missing or present in the target dataset, and split the training set
into two parts accoordingly:
...
The {\tt chopsticks} package includes a number of tools to facilitate
this. All the tests implemented in {\tt chopsticks} are ``score''
...
In {\tt chopsticks}, the statistical testing functions can be
...
this, {\tt chopsticks} provides the {\tt switch.alleles} function, which
