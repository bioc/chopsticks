...
self-explanatory. A word of explanation is necessary concerning the
column headed ``Certain.calls''. We shall see in another vignette
that, when SNP genotypes have not been derived by genotyping but by
{\em imputation} from other SNPs, many imputed values are quite
uncertain; this column gives the proportion of calls that are
uncertain. Here all calls were obtained by genotyping and they are
therefore all denoted as ``certain'', although this does not preclude
...
par(mfrow = c(1, 1))
plot(sample.qc$Call.rate, sample.qc$Heterozygosity)
@ 
