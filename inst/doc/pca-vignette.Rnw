...
in a single {\tt snp.matrix} object. Usually they will
...
which post-multiplies a {\tt snp.matrix} by a general matrix, after first
standardizing the columns of the {\tt snp.matrix} 
