...
pdftitle={PCA-snpMatrix Vignette}]
{hyperref}
...
\title{PCA vignette\\Principal components analysis with snpMatrix}
...
for case/control differences\footnote{An alternative approach is to
  standardise the $X$ matrix so that each column has mean zero in
...
Two comments should be made at this point. Secondly, the data from genome-wide studies
will usually not fit inside a single {\tt snp.matrix} object and will
...
commands extract this variable for the controls and plot a box plot
for the first component by stratum.
<<pc-one,fig=TRUE>>=
...
boxplot(pcs[,2]~pop)
@ 
Clearly the component has captured the difference between the
...
using the special function {\tt snp.pre} which pre-multiplies a {\tt
  snp.matrix} object by a matrix after first standardizing it to zero
mean and unit standard deviation. 
<<pre-multiply>>=
btr <- snp.pre(ctl.10, diag(1/sqrt(evals)) %*% t(pcs))
@ 
...
population structure. To do that we use the function {\tt snp.post}
which post-multiplies a {\tt snp.matrix} by a matrix, after first
standardizing to zero mean and unit standard deviation. Note that it
is first necessary to select out those SNPs that we have actually used
in the calculation of components.
<<post-multiply>>=
pcs <- snp.post(snps.10[,use], t(btr))
@ 
