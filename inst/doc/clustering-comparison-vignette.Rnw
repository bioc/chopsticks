...
\begin{itemize}
\item During the WTCCC study (phase 1, for which {\tt snpMatrix} was written), some multi-panel colourised cluster-plot
...
<<init>>=
library(snpMatrix)
load(system.file("data/Genotypes.GenTrain1.Rda",package="snpMatrix"))
...
<<>>=
ab.signals <- read.wtccc.signals(system.file("data/example-new.txt", package="snpMatrix"),
                                 paste("rs", 1:99, sep=""))
@

We'll just quickly run snpMatrix's SNP summary function to verify:
